...
func CreateOrders(c *fiber.Ctx) error {
	...
	stripe.Key = "sk_test_VQOdfuwdpXljHumJ0umcCf8Z"
...
func CompleteOrder(c *fiber.Ctx) error {
	...
	go func(order models.Order) {
		...
		smtp.SendMail("0.0.0.0:1025", nil, "from-reply@email.com", []string{order.AmbassadorEmail}, ambassadorMessage)

		//send emails
		adminMessage := []byte(fmt.Sprintf("Order $%d has been completed #%f", order.Id, adminRevenue))
		smtp.SendMail("0.0.0.0:1025", nil, "from-reply@email.com", []string{"admin@admin.com"}, adminMessage)
